(function ($) {
    ...
    $(window).on('load', function () {
        ...
        setTimeout(function () {
            $('#preload-content').fadeOut(1200, function () {
                $('#preload').fadeOut(1000);
                setTimeout(function () {
                    $('.fade-in').each(function (index) {
                        $(this).delay(1200 * index).animate({
                            top: 0,
                            opacity: 1
                        }, 800);
                    });
    ...
    $(document).ready(function () {
        ...
        $('#subscribe-form').validate({
            ...
            submitHandler: function (form) {
                $(form).hide();
                $('#subscribe .loading').css({
                    opacity: 0
                }).show().animate({
                    opacity: 1
                });
                $.post($(form).attr('action'), $(form).serialize(), function (data) {
                    $('#subscribe .loading').animate({
                        opacity: 0
                    }, function () {
                        $(this).hide();
                        $('#subscribe .success').show().html('<p>Thank you for subscribing!</p>').animate({
                            opacity: 1
                        });
                    });
                });
